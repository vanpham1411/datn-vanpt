<template>
    <div class="detail food-detail">
        ...
        <div class="add-item">
                ...
                <div class="flex">
                    <div class="w-3/5">
                        <div class="flex">
                            <div class="general-row w-1/2 m-r-12">
                                ...
                                <input :readonly="readonly" v-model="product.code" ref="ProductCode" id="ProductCode"
                                    type="text" maxlength="20" tabindex="2" class="w-full form-control"
                                    :class="{ 'is-invalid': $v.product.code.$error }" @blur="$v.product.code.$touch()">
                                <div v-if="$v.product.code.$error" class="invalid-feedback">
                                    <p v-if="!$v.product.code.required">Mã sản phẩm không được để trống.</p>
                                    <p v-if="($v.product.code.required) && (!$v.product.code.startWithFC)">Mã sản phẩm phải
                                        bắt đầu bằng <i>'FC-'.</i></p>
                                </div>
                            </div>
                            <div class="general-row w-1/2">
                                ...
                                <input :readonly="readonly" v-model="product.name" ref="ProductName" id="ProductName"
                                    type="text" maxlength="50" tabindex="2" class="w-full form-control"
                                    :class="{ 'is-invalid': $v.product.name.$error }" @blur="$v.product.name.$touch()">
                        ...
                        <div class="general-row">
                            ...
                            <vue-combobox @getSelect="categoryModel = $event;" :combobox_valid="false" :placeholder="''"
                                :readonly="readonly" :item_text="['code', 'name']" :display_item="'name'"
                                :items="categoryList" :groupName="['Mã danh mục', 'Tên danh mục']" :vmodel="categoryModel"
                                :multiple="false">
                            </vue-combobox>
                        </div>
                        <div class="general-row">
                        ...
                        <div class="general-row">
                            <p>Mô tả</p>
                            <textarea :readonly="readonly" class="w-full" rows="3" v-model="product.description"
                                placeholder="">
                            </textarea>
                        </div>
                    </div>
                        ...
                        <div class="general-row">
                            ...
                            <div class="center" style="">
                                <div class="flex-center m-r-12" style="min-width: fit-content;">
                                    <input :readonly="readonly" id="upload-file" @change="getFile('#img-Product', $event)"
                                        ref="ImageURL" type="file">
            ...
            <div class="food-table">
                <table>
                    ...
                    <tbody>
                        <tr v-for="(itemDetail, index) in items" :key="index"
                            :class="{ 'list-input': crudDetail != 'read', 'read-input': crudDetail == 'read' }">
                            <td class="center l-30">{{ index + 1 }}</td>

                            <td>
                                <vue-combobox @getSelect="itemDetail.type=$event;" 
                                        :combobox_valid="false" 
                                        :placeholder="''" 
                                        :item_text="['typeID','name']" 
                                        :items="types"
                                        :display_item="'name'" 
                                        :groupName="['Mã loại','Tên loại']"
                                        :vmodel="itemDetail.type"
                                        :readonly="readonly" 
                                        :multiple="false">
                                </vue-combobox>
                            </td>
                            ...
                            <td>
                                <vue-combobox @getSelect="itemDetail.size=$event;" 
                                        :combobox_valid="false" 
                                        :placeholder="''" 
                                        :item_text="['size']" 
                                        :items="sizes"
                                        :groupName="['Kích cỡ']"
                                        :vmodel="itemDetail.size"
                                        :readonly="readonly" 
                                        :multiple="false">
                                </vue-combobox>
                            </td>
                            ...
                            <!-- <td>
                                <vue-combobox @getSelect="itemDetail.size=$event;" 
                                        :combobox_valid="false" 
                                        :placeholder="''" 
                                        :item_text="['sizeID','size']" 
                                        :items="sizes"
                                        :display_item="'size'" 
                                        :groupName="['Mã cỡ','Kích cỡ']"
                                        :vmodel="itemDetail.size"
                                        :readonly="readonly" 
                                        :multiple="false">
                                </vue-combobox>
                            </td> -->
                            <!-- <td class=""><input :readonly="readonly" type="text" style="text-align: end;"
                                    v-model="itemDetail.size"></td> -->

                            <td class=""><input :id="'quantity' + index" :readonly="readonly" type="text"
                                    style="text-align: end;" v-model="itemDetail.quantity"></td>
                            <td class="r-30">
                        ...
                        <tr class="tr-input input-detail" v-if="showInputDetail">
                            ...
                            <!-- <td class=""><input id="typeName" :readonly="readonly" type="text" style="text-align: end;"
                                    v-model="item.type.name"></td>
                            <td class=""><input :readonly="readonly" type="text" style="text-align: end;"
                                    v-model="item.size"></td> -->
                            <td>
                                <vue-combobox @getSelect="item.type=$event;" 
                                        :combobox_valid="false" 
                                        :placeholder="''" 
                                        :item_text="['typeID','name']" 
                                        :items="types"
                            ...
                            <td>
                                <vue-combobox @getSelect="item.size=$event;" 
                                        :combobox_valid="false" 
                                        :placeholder="''" 
                                        :item_text="['size']" 
                                        :items="sizes"
                            ...
                            <td class=""><input id="quantity" :readonly="readonly" type="text" style="text-align: end;"
                                    v-model="item.quantity"></td>

                            <td class="r-30">
                                <div class="delete-icon other-icon logo-icon pointer" @click="removeDetail('item_detail')">
                                </div>
                            </td>
                            <td class="w-30" style="right:-1px; min-width: 31px;"></td>
                        </tr>
        ...
        <div class="form-footer flex" :class="{ 'form-right': !fadeleft }">
            <div class="admin-btn-normal pointer btn-delete" tabindex="18" @click="btnXForm()" @keyup.enter="btnXForm()">Hủy
            </div>
            <div class="btn-save-all flex-center" v-if="crudDetail != 'read'">
                <div class="admin-btn-normal btn-save pointer" tabindex="16" @click="crudObject()"
                    @keyup.enter="crudObject()">Cất</div>
            ...
            <div v-if="crudDetail == 'read'" class="btn-save-all">
                <div class="admin-btn-normal admin-btn-primary pointer" tabindex="16" @click="changeCrud('put')"
                    @keyup.enter="changeCrud('put')">Sửa</div>
            </div>
        </div>
        <div>
            <div v-for="(mgs, index) in listMgs" :key="index">
                <toast-message :titleMgs="mgs.titleMgs" :iconToast="mgs.iconToast" v-if="mgs.showToast"
                    @deleteToast="deleteToast(index)"></toast-message>
            </div>
            <vue-popup v-if="popShow" :popShow="popShow" :action="action" :idInvalid="idInvalid" @closeForm="closeForm"
                :message="message" :crud="crudDetail" @closePop="closePop" @crudObject="crudObject()"></vue-popup>
        </div>
    </div>
</template>
...
import {ListStringSize} from '../../../../base/vi/Resources';
export default {
    ...
    async created() {
        try {
            ...
            if (this.ProductCode) {
                ...
                let data= (await ProductAPI.getAllType());
                this.types = data;
                // console.log("get all type: ", this.types);
                // console.log("get all sizes: ", this.sizes);
            }
            else {
    ...
    methods: {
        ...
        */
                ...
                for (let key in this.product) {
                    if (this.$v.product[key]) {
                        if (this.$v.product[key].$error == true) {
                            this.idInvalid = "Product" + key.charAt(0).toUpperCase() + key.slice(1);
                            console.log("invalid show popup: ", this.idInvalid)
                            //console.log("invalid show popup: ", document.querySelector("#"+this.idInvalid+" ~ div > p").textContent)
                            console.log("this.$refs[this.idInvalid]" , this.$refs[this.idInvalid])
                    ...
                    if (this.validateForm() == true) {
                        ...
                        if ((this.crudDetail == CRUD.Post) || (this.crudDetail == CRUD.Copy)) {
                            ...
                            // nếu không thêm mới thành công
                                ...
                                this.message = "Mã sản phẩm đã tồn tại";
        ...
        getFile(imgSelector, event) {
            console.log("get file: ",imgSelector, event)
